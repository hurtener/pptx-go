...
| `Grid` | `Columns int (2..4); Ratio []int; Gap SpaceRole; Cells []SlideNode; Connectors []GridConnector; Fill bool` | cell count must be a multiple of `Columns`; `Ratio` empty or len == `Columns`. `Connectors` (`{Between [2]int; Kind ConnectorKind; Label string}`) draw a glyph in the gutter between two adjacent columns (`Between` = `{c, c+1}`) — reuse the flow `ConnectorArrow`/`ConnectorBiArrow`/… set — so an architecture/pipeline grid reads as flow; empty = none, byte-identical. `Fill` (opt-in, zero=false) lets the grid claim leftover body-stack height under `VAlignTop` without turning on slide-wide `VAlignFill`; the taller slot is then subdivided into taller equal-height rows. Under any slide VAlign other than `VAlignTop` the field is inert in V1 |
| `Bento` | `Columns int (≥1); Rows []BentoRow` (`{Label string; Cells []BentoCell}`, `BentoCell{Span int; Node SlideNode}`); `WeightedRows bool`; `Fill bool` | row-labeled grid: rows with an optional left label and cells of variable column span on a shared grid (a span-S cell = S of `Columns` units). A row's spans sum to ≤ `Columns`; the gutter is reserved only when some row has a `Label`. `WeightedRows` (opt-in) sizes each row to its content's preferred height — dense rows grow, sparse rows shrink, clamped so the bento always fits its region; default equal rows are byte-identical. `Fill` (opt-in, zero=false) lets the bento claim leftover body-stack height under `VAlignTop` without slide-wide `VAlignFill`; the taller slot is then subdivided by the existing bento geometry into taller rows/cells. Under other slide VAlign modes the field is inert in V1 |
