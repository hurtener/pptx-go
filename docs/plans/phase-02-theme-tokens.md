...
The new `pptx` files are banded in `coverage.json` for this phase; the
inherited `pptx` files stay outside the gate until the Phase 03 rewrite
(D-029-style staging). `require_configured` stays `false`.
...
- *(empty until implementation — D-030 is a planned decision)*
