...
func TestOrnamentRecipes_EmitShapes(t *testing.T) {
	...
	for name, rec := range curated() {
		t.Run(name, func(t *testing.T) {
			n1 := rec(newSlide(t), box, pptx.AlphaOpaque, 0, pptx.ColorAccent, 0)
			if n1 < 1 {
				t.Fatalf("%s emitted %d shapes, want >= 1", name, n1)
			}
			if n2 := rec(newSlide(t), box, pptx.AlphaOpaque, 0, pptx.ColorAccent, 0); n2 != n1 {
...
func TestChevronArrow_Rotation(t *testing.T) {
	...
	if n := assetornaments.ChevronArrow(newSlide(t), box, pptx.AlphaOpaque, 90, pptx.ColorAccent, 0); n != 1 {
